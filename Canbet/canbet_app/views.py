...
# ═══════════════════════════════════════════════════════════════════════════════
#  PAGE VIEWS  (serve the HTML templates)
# ═══════════════════════════════════════════════════════════════════════════════
...
def main(request):
    ...
    recent  = list(user.crate_opens.select_related('item_won')[:5]) + \
              list(user.purchases.select_related('item')[:5])
    # sort mixed activity by timestamp
    recent.sort(key=lambda x: getattr(x, 'opened_at', None) or getattr(x, 'purchased_at', None), reverse=True)
...
def inventory(request):
    entries = request.user.inventory.select_related('item').order_by('-obtained_at')
...
# ═══════════════════════════════════════════════════════════════════════════════
#  REST API  (called by JS fetch() in the HTML pages)
# ═══════════════════════════════════════════════════════════════════════════════
...
def api_me(request):
    ...
    return Response({
        'username':      u.username,
        'bit_balance':   u.bit_balance,
        'crates_opened': u.crates_opened,
        'rank':          u.rank,
    })
...
def api_open_crate(request):
    ...
    # weighted random roll from the crate's collection
    pool = list(Item.objects.filter(collection=crate_type))
    if not pool:
        return Response({'error': 'Crate pool is empty.'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
    ...
    with transaction.atomic():
        ...
        CrateOpen.objects.create(
            user=user, crate_type=crate_type, item_won=won, bits_spent=cost
        )
...
def api_inventory(request):
    entries = request.user.inventory.select_related('item')
    return Response([
        {
            'item_id':     e.item.id,
            'name':        e.item.name,
...
def api_leaderboard(request):
    players = CanBetUser.objects.order_by('-bit_balance')[:50]
    return Response([
        {
            'rank':          idx + 1,
            'username':      p.username,
            'bit_balance':   p.bit_balance,
            'crates_opened': p.crates_opened,
        }
        for idx, p in enumerate(players)
    ])


# ── GET /api/recent-opens/ ────────────────────────────────────────────────────
@api_view(['GET'])
